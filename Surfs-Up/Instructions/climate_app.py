...
def start_date(start):
    ...
    """ Retrieve the precipitation data from start date on. """
    temps = session.query(Measurement.date, Measurement.tobs).filter(Measurement.date >= start)

    """ Save the query results as a Pandas DataFrame and set the index to the date column """
    temps_df = pd.DataFrame(temps[:], columns = ["date", "tobs"]).dropna()
    
    message_dict  = {"Temperature stats for date: ": start}
    ...
    return jsonify(message_dict, temps_dict)
